...
// Kiểu dữ liệu cho mỗi ngày trong kỳ công
export interface PeriodDay {
  date: Date;
  day: number;
  month: number;
  year: number;
  key: string;   // "2026-06-26" — dùng làm key records
  label: string; // "26/06" — hiển thị
  dow: string;   // "Thứ 2" — hiển thị
}
...
// Helper: xác định cuối tuần từ tên thứ
const isSunday = (dow: string) => dow === "Chủ nhật";
const isWeekend = (dow: string) => dow === "Chủ nhật" || dow === "Thứ 7";

// ===== HÀM SUY RA TRẠNG THÁI CÔNG TỪ KPI =====
// - Có Mini Show HOẶC Big Show → "X" (tính công)
// - Không có gì → "OFF"
            ...
            <table className="w-full text-sm border-collapse min-w-[2000px]">
              <thead className="sticky top-0 z-30">
                <tr className="bg-slate-900 text-white">
                  ...
                  {daysArray.map((d) => {
                    const sunday = isSunday(d.dow);
                    const weekend = isWeekend(d.dow);
                    return (
                      <th
                        key={d.key}
                        className={`p-2 text-center min-w-[90px] border-r border-slate-700 font-bold ${
                          sunday ? "bg-red-900/80 text-red-100" : weekend ? "bg-slate-800 text-slate-200" : ""
                        }`}
                      >
                        {d.label}
                      </th>
                ...
                <tr className="bg-slate-800 text-xs">
                  {daysArray.map((d) => {
                    ...
                    return (
                      <th
                        key={`sub-${d.key}`}
                        className={`p-1.5 text-center border-r border-slate-700 font-medium ${
                          sunday
              ...
              <tbody className="bg-white divide-y divide-slate-200/60">
                {sortedAttendances.map((att, idx) => {
                  ...
                  daysArray.forEach((d) => {
                    const rawVal = rowData.records?.[d.key];
                    const val =
                      rawVal === undefined || rawVal === null
                        ? ""
                        : rawVal === "x" || rawVal === "Chạm"
                        ? "X"
                        : rawVal;
                  ...
                  return (
                    ...
                    >
                      <td
                        className={`p-3 border-r border-slate-200/60 sticky left-0 z-20 shadow-[1px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors ${
                      ...
                      {daysArray.map((d) => {
                        ...
                        // Style cho text trạng thái read-only
                        let statusTextStyle = "text-slate-300";
                        let statusLabel = "OFF";
                        if (derivedStatus === "X") {
                        ...
                        return (
                          <td
                            key={d.key}
                            className={`p-1.5 border-r border-slate-200/60 align-top transition-colors ${
                              sunday ? "bg-red-50/20" : ""
                            }`}
                          >
                            <div className="flex flex-col gap-1 w-full h-full">
                              {/* Text trạng thái read-only - suy ra từ KPI */}
                              <div className={`w-full h-5 flex items-center justify-center text-[10px] font-bold uppercase ${statusTextStyle}`}>
                                {statusLabel}
                              </div>

                              {/* 2 ô input Mini Show / Big Show */}
                              <div className="grid grid-cols-2 gap-1">
                                <input
                                  type="text"
                                  maxLength={2}
                                  title="Mini Show"
                                  className="w-full h-7 text-xs font-semibold text-center border rounded-md outline-none transition-all duration-200 border-slate-200 hover:border-cyan-400 focus:bg-cyan-50 focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 text-cyan-700 bg-white"
                                  placeholder="M"
                                ...
                                <input
                                  ...
                                  maxLength={2}
                                  title="Big Show"
                                  className="w-full h-7 text-xs font-semibold text-center border rounded-md outline-none transition-all duration-200 border-slate-200 hover:border-purple-400 focus:bg-purple-50 focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 text-purple-700 bg-white"
                                  placeholder="B"
                      ...
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-blue-700 bg-blue-50/50 text-sm">
                        {calcFull}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-amber-600 bg-amber-50/50 text-sm">
                        {calcHalf}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-black text-amber-700 bg-amber-100/50 text-sm">
                        {calcPaid}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-cyan-700 bg-cyan-50/50 text-sm">
                        {calcMini}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-purple-700 bg-purple-50/50 text-sm">
                        {calcBig}
                      </td>

                      <td
                        className={`p-2 sticky right-0 z-20 shadow-[-1px_0_5px_-2px_rgba(0,0,0,0.1)] transition-colors ${
                          idx % 2 === 0 ? "bg-white group-hover:bg-blue-50" : "bg-slate-50 group-hover:bg-blue-50"
                        }`}
                      >
                        {isEditing && !isClosed ? (
...
export const OvertimeTable: React.FC<SharedProps & { toggleOvertime: any }> = ({
  isDataLoading,
  filteredAttendances,
  editingRecords,
  selectedMonthDoc,
                ...
                <tr className="bg-teal-900 text-white">
                  ...
                  {daysArray.map((d) => {
                    ...
                    return (
                      <th
                        key={d.key}
                        className={`p-2 text-center min-w-[45px] border-r border-teal-700 font-bold text-xs ${
                          sunday ? "bg-red-800/80 text-red-100" : weekend ? "bg-teal-800 text-teal-100" : ""
                        }`}
                      >
                        {d.label}
                      </th>
                  ...
                  <th rowSpan={2} className="p-2.5 bg-teal-800 sticky right-0 z-40 min-w-[90px] font-semibold text-center">
                    Thao tác
                  </th>
                </tr>
                <tr className="bg-teal-800 text-xs">
                  {daysArray.map((d) => {
                    ...
                    return (
                      <th
                        key={`sub-ot-${d.key}`}
                        className={`p-1.5 text-center font-medium border-r border-teal-700 ${
                          sunday
                            ? "text-red-300 bg-red-900/40"
                            : weekend
                            ? "text-teal-200 bg-teal-700/50"
                            : "text-teal-300"
                        }`}
                      >
                        {d.dow}
                  ...
                  return (
                    ...
                    >
                      ...
                      {daysArray.map((d) => {
                        ...
                        let cellStyle = "text-slate-400 font-normal",
                          bgStyle = "hover:bg-slate-100 bg-white";
                        if (val === "X") {
                          cellStyle = "text-blue-700 font-bold";
                          bgStyle = "bg-blue-100 border-blue-300 hover:bg-blue-200";
                        } else if (val === "N") {
                          cellStyle = "text-amber-600 font-bold";
                          bgStyle = "bg-amber-100 border-amber-300 hover:bg-amber-200";
                        } else if (val === "T") {
                          cellStyle = "text-purple-700 font-bold";
                          bgStyle = "bg-purple-100 border-purple-300 hover:bg-purple-200";
                        }

                        return (
                          <td
                            key={d.key}
                            className={`p-1 border-r border-slate-200/60 text-center transition-colors ${
                              sunday ? "bg-red-50/20" : ""
                            }`}
                          >
                            <button
                      ...
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-blue-700 text-sm bg-blue-50/30">
                        {calcOTN}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-amber-600 text-sm bg-amber-50/30">
                        {calcOTW}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-bold text-purple-700 text-sm bg-purple-50/30">
                        {calcOTH}
                      </td>
                      <td className="p-3 border-r border-slate-200/60 text-center font-black text-emerald-800 bg-emerald-100/50 text-sm">
                        {calcTotalOT}
                      </td>
...
export const ShortfallTable: React.FC<SharedProps & { handleShortfallDailyChange: any }> = ({
  isDataLoading,
  filteredAttendances,
  editingRecords,
  selectedMonthDoc,
                ...
                <tr className="bg-rose-950 text-white">
                  ...
                  {daysArray.map((d) => {
                    ...
                    return (
                      <th
                        key={d.key}
                        className={`p-2 text-center min-w-[50px] border-r border-rose-800 font-bold text-xs ${
                          sunday ? "bg-red-800/80 text-red-100" : weekend ? "bg-rose-900 text-rose-100" : ""
                        }`}
                      >
                        {d.label}
                      </th>
                  ...
                  <th rowSpan={2} className="p-2 bg-rose-900 sticky right-0 z-40 min-w-[90px] font-semibold text-center">
                    Thao tác
                  </th>
                </tr>
                <tr className="bg-rose-900 text-xs">
                  {daysArray.map((d) => {
                    ...
                    return (
                      <th
                        key={`sub-sf-${d.key}`}
                        className={`p-1.5 text-center font-medium border-r border-rose-800 ${
                          sunday
                            ? "text-red-300 bg-red-900/40"
                            : weekend
                            ? "text-rose-200 bg-rose-800/50"
                            : "text-rose-300"
                        }`}
                      >
                        {d.dow}
                    ...
                    >
                      ...
                      {daysArray.map((d) => {
                        ...
                        return (
                          <td
                            key={d.key}
                            className={`p-1 border-r border-slate-200/60 text-center ${sunday ? "bg-red-50/20" : ""}`}
                          >
                            <input
                              type="text"
                              disabled={isClosed}
