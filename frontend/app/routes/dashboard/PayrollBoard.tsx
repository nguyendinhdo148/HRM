...
const TabPayrollTable = ({ filteredPayrolls, editingRecords, isClosed, handleInputChange, handleSaveRow }: any) => {
  return (
        ...
        <table className="w-full text-[11px] border-collapse min-w-[2600px]">
          <thead className="bg-[#003366] text-white">
            <tr>
              ...
              <th rowSpan={2} className="p-2 border-r border-slate-600 bg-[#1d4ed8] text-center max-w-[100px]">Làm Thêm Giờ</th>
              <th colSpan={3} className="p-2 border-r border-slate-600 bg-[#6b21a8] text-center">Hiệu Suất (Show & Công)</th>
              <th colSpan={6} className="p-2 border-r border-slate-600 bg-[#0f766e] text-center">Các Khoản Phụ Cấp</th>
              <th rowSpan={2} className="p-2 border-r border-slate-600 bg-amber-600 text-center max-w-[100px]">Thưởng (+)</th>
              ...
              <th rowSpan={2} className="p-2 border-r border-slate-600 bg-[#831843] text-center max-w-[80px]">Tạm Ứng</th>
              <th colSpan={3} className="p-2 border-r border-slate-600 bg-[#9f1239] text-center">Bảo Hiểm & Thuế</th>
            ...
            <tr className="bg-[#1e293b] text-[10px] text-center text-slate-300">
              ...
              <th className="p-1 border-r border-slate-600 text-[#d8b4fe]">Mini Show</th>
              <th className="p-1 border-r border-slate-600 text-[#d8b4fe]">Big Show</th>
              <th className="p-1 border-r border-slate-600 text-[#d8b4fe]">Thưởng N.Công</th>
              ...
              <th className="p-1 border-r border-slate-600 text-rose-200">BH (NLĐ)</th>
            ...
            {filteredPayrolls.map((p: any, idx: number) => {
              ...
              return (
                <tr key={p._id} className={`${idx % 2 === 0 ? "bg-white" : "bg-[#f8fafc]"} hover:bg-blue-50/50 transition-colors`}>
                  ...
                  <td className="p-2 border-r border-slate-200 text-right font-bold text-purple-700 bg-purple-50/20">{formatNumberWithDot(p.incomes?.miniShowMoney)}</td>
                  <td className="p-2 border-r border-slate-200 text-right font-bold text-purple-700 bg-purple-50/20">{formatNumberWithDot(p.incomes?.bigShowMoney)}</td>
                  <td className="p-2 border-r border-slate-200 text-right font-bold text-purple-800 bg-purple-100/30">{formatNumberWithDot(p.incomes?.kpiBonus)}</td>
                  ...
                  <td className="p-1 border-r border-slate-200 bg-amber-50/50">
                    <input
                      ...
                      onChange={(e) => handleInputChange(p._id, "bonus", e.target.value)} placeholder="0"
                    />
                  </td>
...
// ... COMPONENT KÊ KHAI THUẾ (Giữ nguyên)
const TabTaxSummary = () => {
  ...
  const fetchAndAggregate = async () => {
    ...
    try {
      ...
      const promises = monthsToFetch.map(m => fetch(`${API_BASE_URL}?month=${m}&year=${year}`, { headers: getAuthHeaders() }).then(res => res.json()));
      ...
      allRecords.forEach((r: any) => {
        ...
        if (!summaryMap[code]) summaryMap[code] = { employeeCode: code, fullName: r.employeeSnapshot.fullName, department: r.employeeSnapshot.department, totalGross: 0, totalTax: 0, totalInsurance: 0, totalNet: 0 };
  ...
  const handleExportTaxExcel = () => {
    ...
    aggregatedData.forEach((row, index) => {
      wsData.push([
        { v: index + 1, s: cellCenter }, { v: row.employeeCode, s: cellCenter }, { v: row.fullName, s: cellLeft }, { v: row.department || "", s: cellCenter },
        { v: formatNumberWithDot(row.totalGross), s: cellRight }, { v: formatNumberWithDot(row.totalTax), s: cellRight }, { v: formatNumberWithDot(row.totalInsurance), s: cellRight }, { v: formatNumberWithDot(row.totalNet), s: cellRight },
      ]);
    });
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex flex-col">
            ...
            <select className="border rounded-xl px-3 py-2 text-sm bg-slate-50 font-bold outline-none cursor-pointer" value={period} onChange={(e) => setPeriod(e.target.value)}>
              <option value="ALL">Cả Năm</option><option value="Q1">Quý 1</option><option value="Q2">Quý 2</option><option value="Q3">Quý 3</option><option value="Q4">Quý 4</option>
            </select>
          </div>
        </div>
...
export default function PayrollBoard() {
  ...
  const [configRates, setConfigRates] = useState({
    minishow:  40800 
, bigshow:  130500 
, meal: 35000, transport: 30000, housingUnder15: 425000, housingOver15: 850000
  });
  ...
  const handleExportExcel = () => {
    ...
    filteredPayrolls.forEach((p, index) => {
      ...
      wsData.push([
        { v: index + 1, s: cellCenter }, { v: snap.fullName || "", s: cellLeft }, { v: snap.employeeCode || "", s: cellCenter }, { v: snap.position || "", s: cellCenter }, { v: snap.department || "", s: cellCenter }, { v: fmt(p.baseSalary), s: cellRight }, { v: p.actualDays || 0, s: cellCenter }, { v: fmt(inc.timeSalary), s: cellRight }, { v: fmt(inc.overtime), s: cellRight }, { v: fmt(inc.miniShowMoney), s: cellRightBold }, { v: fmt(inc.bigShowMoney), s: cellRightBold }, { v: fmt(inc.kpiBonus), s: cellRightBold }, { v: fmt(allw.meal), s: cellRight }, { v: fmt(allw.transport), s: cellRight }, { v: fmt(allw.phone), s: cellRight }, { v: fmt(allw.clothing), s: cellRight }, { v: fmt(allw.housing), s: cellRight }, { v: fmt(totalAllw), s: cellRightBold }, { v: fmt(inc.bonus), s: cellRight }, { v: fmt(inc.totalGross), s: cellRightBold }, { v: fmt(ded.advance), s: cellRight }, { v: fmt(ins.total), s: cellRight }, { v: fmt(ded.taxTNCN), s: cellRight }, { v: fmt(ded.totalDeductions), s: cellRightBold }, { v: fmt(p.netSalary), s: { ...cellRightBold, font: { ...FONT, bold: true, color: { rgb: "C00000" } } } }, { v: "", s: cellLeft }
      ]);
    });
    ...
    ws["!cols"] = [{ wch: 5 }, { wch: 22 }, { wch: 10 }, { wch: 15 }, { wch: 15 }, { wch: 12 }, { wch: 10 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 11 }, { wch: 11 }, { wch: 11 }, { wch: 11 }, { wch: 11 }, { wch: 12 }, { wch: 12 }, { wch: 15 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 12 }, { wch: 15 }, { wch: 15 }];
    const wb = XLSX.utils.book_new();
        ...
        <Card className="shadow-sm border-none bg-white">
          ...
          <CardContent className="p-4 pt-0 space-y-3">
            <div className="bg-slate-50 p-3 rounded-xl border space-y-2 mb-2">
              <div className="flex items-center gap-1 mb-2 font-bold text-xs text-slate-600"><Settings className="w-3 h-3"/> Thông số tính toán (VNĐ)</div>
              <div className="grid grid-cols-2 gap-2 text-[10px]">
                ...
                <div className="col-span-2 flex gap-2">
                  <div className="flex-1"><label className="text-slate-500">Ở (≤ 15 ngày)</label><Input type="number" value={configRates.housingUnder15} onChange={(e) => setConfigRates({...configRates, housingUnder15: Number(e.target.value)})} className="h-6 text-[10px] px-1" /></div>
                  <div className="flex-1"><label className="text-slate-500">Ở (&gt; 15 ngày)</label><Input type="number" value={configRates.housingOver15} onChange={(e) => setConfigRates({...configRates, housingOver15: Number(e.target.value)})} className="h-6 text-[10px] px-1" /></div>
                </div>
              </div>
            </div>
        ...
        {selectedMonthDoc ? (
          <Tabs defaultValue="board">
            ...
            <TabsContent value="board" className="mt-0">
              ...
              {isDataLoading ? (<div className="bg-white h-64 rounded-2xl flex items-center justify-center"><Loader /></div>) : (<TabPayrollTable filteredPayrolls={filteredPayrolls} editingRecords={editingRecords} isClosed={selectedMonthDoc.status !== "draft"} handleInputChange={handleInputChange} handleSaveRow={handleSaveRow} />)}
            </TabsContent>
          ...
          <div className="h-[600px] flex flex-col items-center justify-center text-slate-400 bg-white/50 backdrop-blur-sm rounded-3xl border-2 border-dashed border-slate-200 shadow-sm">
            <Calculator className="w-20 h-20 mb-4 opacity-20" /><p className="font-bold text-lg uppercase tracking-widest opacity-60">Vui lòng khởi tạo một bảng lương</p>
          </div>
        )}
      </div>
